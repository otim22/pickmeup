...
class ResturantLocationQuerySet(models.query.QuerySet):
    def search(self, query):       # ResturantLocation.objects.all().search(query)  
        if query:
            query = query.strip()
            return self.filter(
...
class ResturantLocationManager(models.Manager):
    def get_queryset(self):
        return ResturantLocationQuerySet(self.model, using=self._db)

    def search(self, query):          # ResturantLocation.objects.search()
...
class ResturantLocation(models.Model):
    ...
    objects = ResturantLocationManager()
